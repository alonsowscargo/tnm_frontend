...
import {
  deleteRegisterCuentasBancarias,
  getRegisterCuentasBancarias,
} from 'actions/CuentasBancariasMaintainer';

const MantenedorCuentasBancariasListado = (props) => {
  ...
  return (
    <div>
        <div className="bg-primary Form_Cabecera mt-3">
            <IntlMessages id="label.listadoCuentasBancarias"/>
          </div>
          <div className="border rounded">
          <MaterialTable
            title=''
            columns={state.columns}
            data={propState.cuentasBancariasList}
            localization={{
              header: {
            ...
            actions={[
              {
                ...
                onClick: (event, rowData) => {
                  dispatch(getRegisterCuentasBancarias(rowData));
                }
              },
              {
                icon: props => (
                ...
                tooltip: 'Eliminar',
                onClick: (event, rowData) => {
                  dispatch(deleteRegisterCuentasBancarias(rowData));
                }
              }
            ]}
            options={{
...
export default MantenedorCuentasBancariasListado;
