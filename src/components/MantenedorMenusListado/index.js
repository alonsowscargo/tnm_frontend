...
const MantenedorMenusListado = (props) => {
  ...
  const [state, setState] = useState({
    columns: [
      ...
      { title: <IntlMessages id="label.permiso"/>, field: 'permiso' },
      { title: <IntlMessages id="label.menuPadre"/>, field: 'menuPadre' },
      { title: <IntlMessages id="label.url"/>, field: 'url' },
    ],
  });
  ...
  return (
    <div>
        <div className="bg-primary Form_Cabecera mt-3">
            <IntlMessages id="label.listadoMenus"/>
          </div>
          <div className="border rounded">
          <MaterialTable
            title=''
            columns={state.columns}
            data={propState.menuList}
            localization={{
              header: {
            ...
            actions={[
              {
                ...
                onClick: (event, rowData) => {
                  dispatch(getRegisterMenu(rowData));
                }
              },
              {
                icon: props => (
                ...
                tooltip: 'Eliminar',
                onClick: (event, rowData) => {
                  dispatch(deleteRegisterMenu(rowData));
                }
              }
            ]}
            options={{
...
export default MantenedorMenusListado;
