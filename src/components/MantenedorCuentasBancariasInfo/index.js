...
import {
  onChangePayload,
  postRegisterCuentasBancarias,
  putRegisterCuentasBancarias
} from 'actions/CuentasBancariasMaintainer';

const MantenedorCuentasBancariasInfo = (props) => {

  const dispatch = useDispatch();
  const [controls, setControls] = useState(props.payload);

  const [cuentasBancarias, setCuentasBancarias] = useState(props);
  useEffect(() => {
    setCuentasBancarias(props);
  ...
  return (
    <div>
        <div className="bg-primary Form_Cabecera">
          <IntlMessages id="label.InformacionCuentasBancarias"/>
        </div>
        <div className="Form_Cuerpo">
            ...
            <Button variant="contained" className="bg-primary text-white jr-btn jr-btn-sm mt-3 rounded-lg" onClick={()=>{
              if(controls.id>0){
                dispatch(putRegisterCuentasBancarias(controls))
              }else{
                dispatch(postRegisterCuentasBancarias(controls))
              }
              }}>
...
export default MantenedorCuentasBancariasInfo;
