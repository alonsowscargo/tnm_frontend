...
import {
  deleteRegisterBancos, 
  getRegisterBancos,
} from 'actions/BancosMaintainer';

const MantenedorBancosListado = (props) => {
  ...
  return (
    <div>
        <div className="bg-primary Form_Cabecera mt-3">
            <IntlMessages id="label.listadoBancos"/>
          </div>
          <div className="border rounded">
          <MaterialTable
            title=''
            columns={state.columns}
            data={propState.bancosList}
            localization={{
              header: {
            ...
            actions={[
              {
                ...
                onClick: (event, rowData) => {
                  dispatch(getRegisterBancos(rowData));
                }
              },
              {
                icon: props => (
                ...
                tooltip: 'Eliminar',
                onClick: (event, rowData) => {
                  dispatch(deleteRegisterBancos(rowData));
                }
              }
            ]}
            options={{
...
export default MantenedorBancosListado;
