...
import {
  deleteRegisterMonedas,
  getRegisterMonedas,
} from 'actions/MonedasMaintainer';

const MantenedorMonedasListado = (props) => {
  ...
  return (
    <div>
        <div className="bg-primary Form_Cabecera mt-3">
            <IntlMessages id="label.listadoMonedas"/>
          </div>
          <div className="border rounded">
          <MaterialTable
            title=''
            columns={state.columns}
            data={propState.monedasList}
            localization={{
              header: {
            ...
            actions={[
              {
                ...
                onClick: (event, rowData) => {
                  dispatch(getRegisterMonedas(rowData));
                }
              },
              {
                icon: props => (
                ...
                tooltip: 'Eliminar',
                onClick: (event, rowData) => {
                  dispatch(deleteRegisterMonedas(rowData));
                }
              }
            ]}
            options={{
...
export default MantenedorMonedasListado;
