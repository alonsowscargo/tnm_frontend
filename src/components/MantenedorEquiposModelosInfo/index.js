...
import {
  onChangePayload,
  postRegisterEquiposModelos,
  putRegisterEquiposModelos
} from 'actions/EquiposModelosMaintainer';

const MantenedorEquiposModelosInfo = (props) => {

  const dispatch = useDispatch();
  const [controls, setControls] = useState(props.payload);

  const [equiposModelos, setEquiposModelos] = useState(props);
  useEffect(() => {
    setEquiposModelos(props);
  ...
  return (
    <div>
        <div className="bg-primary Form_Cabecera">
          <IntlMessages id="label.InformacionEquiposModelos"/>
        </div>
        <div className="Form_Cuerpo">
            ...
            <Button variant="contained" className="bg-primary text-white jr-btn jr-btn-sm mt-3 rounded-lg" onClick={()=>{
              if(controls.id>0){
                dispatch(putRegisterEquiposModelos(controls))
              }else{
                dispatch(postRegisterEquiposModelos(controls))
              }
              }}>
...
export default MantenedorEquiposModelosInfo;
