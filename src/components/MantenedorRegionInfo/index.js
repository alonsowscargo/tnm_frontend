...
const MantenedorRegionInfo = (props) => {

  const dispatch = useDispatch();
  const [controls, setControls] = useState(props.payload);

  const [regions, setRegions] = useState(props);
  useEffect(() => {
    setRegions(props);
        ...
        <div className="Form_Cuerpo">
            <div className="row">
              ...
              <Button variant="contained" className="bg-primary text-white jr-btn jr-btn-sm mt-3 rounded-lg" onClick={()=>{
                if(controls.id>0){
                  dispatch(putRegisterRegion(controls))
                }else{
                  dispatch(postRegisterRegion(controls))
                }
                }}>
...
export default MantenedorRegionInfo;
