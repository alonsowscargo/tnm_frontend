...
import {
  onChangePayload,
  postRegisterEquiposTipos,
  putRegisterEquiposTipos
} from 'actions/EquiposTiposMaintainer';

const MantenedorEquiposTiposInfo = (props) => {

  const dispatch = useDispatch();
  const [controls, setControls] = useState(props.payload);

  const [equiposTipos, setEquiposTipos] = useState(props);
  useEffect(() => {
    setEquiposTipos(props);
  ...
  return (
    <div>
        <div className="bg-primary Form_Cabecera">
          <IntlMessages id="label.InformacionEquiposTipos"/>
        </div>
        <div className="Form_Cuerpo">
            ...
            <Button variant="contained" className="bg-primary text-white jr-btn jr-btn-sm mt-3 rounded-lg" onClick={()=>{
              if(controls.id>0){
                dispatch(putRegisterEquiposTipos(controls))
              }else{
                dispatch(postRegisterEquiposTipos(controls))
              }
              }}>
...
export default MantenedorEquiposTiposInfo;
