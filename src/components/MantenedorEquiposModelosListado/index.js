...
import {
  deleteRegisterEquiposModelos, 
  getRegisterEquiposModelos,
} from 'actions/EquiposModelosMaintainer';

const MantenedorEquiposModelosListado = (props) => {
  ...
  return (
    <div>
        <div className="bg-primary Form_Cabecera mt-3">
            <IntlMessages id="label.listadoEquiposModelos"/>
          </div>
          <div className="border rounded">
          <MaterialTable
            title=''
            columns={state.columns}
            data={propState.equiposModelosList}
            localization={{
              header: {
            ...
            actions={[
              {
                ...
                onClick: (event, rowData) => {
                  dispatch(getRegisterEquiposModelos(rowData));
                }
              },
              {
                icon: props => (
                ...
                tooltip: 'Eliminar',
                onClick: (event, rowData) => {
                  dispatch(deleteRegisterEquiposModelos(rowData));
                }
              }
            ]}
            options={{
...
export default MantenedorEquiposModelosListado;
