...
import {
  deleteRegisterComunas,
  getRegisterComunas,
} from 'actions/ComunasMaintainer';
import {
  getRegisterRegionByComunaList,
} from 'actions/RegionMaintainer';

const MantenedorComunasListado = (props) => {
  ...
  const [state, setState] = useState({
    columns: [
      ...
      { title: <IntlMessages id="label.region"/>, field: 'region_nombre' },
    ],
  });
  ...
  return (
    <div>
        <div className="bg-primary Form_Cabecera mt-3">
            <IntlMessages id="label.listadoComunas"/>
          </div>
          <div className="border rounded">
          <MaterialTable
            title=''
            columns={state.columns}
            data={propState.comunasList}
            localization={{
              header: {
            ...
            actions={[
              {
                ...
                onClick: (event, rowData) => {
                  dispatch(getRegisterComunas(rowData));
                  dispatch(getRegisterRegionByComunaList(rowData));
                }
              },
              {
                icon: props => (
                ...
                tooltip: 'Eliminar',
                onClick: (event, rowData) => {
                  dispatch(deleteRegisterComunas(rowData));
                }
              }
            ]}
            options={{
...
export default MantenedorComunasListado;
