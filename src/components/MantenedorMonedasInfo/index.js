...
const MantenedorMonedasInfo = (props) => {

  const dispatch = useDispatch();
  const [controls, setControls] = useState(props.payload);

  const [monedas, setMonedas] = useState(props);
  useEffect(() => {
    setMonedas(props);
  ...
  return (
    <div>
        <div className="bg-primary Form_Cabecera">
          <IntlMessages id="label.InformacionMonedas"/>
        </div>
        <div className="Form_Cuerpo">
            ...
            <Button variant="contained" className="bg-primary text-white jr-btn jr-btn-sm mt-3 rounded-lg" onClick={()=>{
              if(controls.id>0){
                dispatch(putRegisterMonedas(controls))
              }else{
                dispatch(postRegisterMonedas(controls))
              }
              }}>
...
export default MantenedorMonedasInfo;
